...
class ListViewAjax
{
     ...
     * @return    string
     */
    function __construct($typeid, $uppage=1,$orderby_ext="",$orderway_ext="")
    {
        global $dsql,$ftp;
        $this->TypeID = $typeid;
        $this->dsql = &$dsql;
        ...
        $this->ftp = &$ftp;
        $this->remoteDir = '';
        $this->orderby_ext = $orderby_ext;
        $this->orderway_ext = $orderway_ext;
    ...
    function CountRecord()
    {
        ...
        $tempfile=str_replace(".htm","_ajax.htm",$tempfile);//dedediy
    ...
    $innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
        ...
        //排序方式
        $ordersql = '';
        $addtable  = $this->ChannelUnit->ChannelInfos['addtable'];//dedediy
        if($this->orderby_ext!=""){
            $orderby = $this->orderby_ext;
        }
        if($this->orderway_ext!=""){
            $orderWay = $this->orderway_ext;
        }
        if($orderby=="senddate" || $orderby=="id") {
            ...
            $ordersql = "  ORDER BY arc.zan $orderWay";
        }
        else if($orderby=="trueprice") {
            $ordersql = "  ORDER BY ".$addtable.".trueprice $orderWay";
        }
        else {
        ...
        if(preg_match('/hot|click|lastpost|scores|weight|zan|trueprice/', $orderby))
        {
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        $geturl = "tid=".$this->TypeID."&TotalResult=".$this->TotalResult."&orderby=".$this->orderby_ext."&orderway=".$this->orderway_ext."&";
        $purl .= '?'.$geturl;
